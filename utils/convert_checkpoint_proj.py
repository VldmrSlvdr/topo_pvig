#!/usr/bin/env python
"""
Convert a baseline model checkpoint to be partially compatible with the projection model architecture.
This script extracts transferable parameters from a baseline model and saves them in a format
suitable for partial loading into the projection model.

Usage:
    python convert_checkpoint_proj.py input_checkpoint.pth output_pretrained.pth.tar
"""
...
def convert_baseline_to_proj(input_path, output_path, verbose=True):
    """
    Convert a baseline model checkpoint to be compatible with projection model.
    ...
    # Create new state dict for projection model
    ...
    is_baseline_model = any('backbone.' in k for k in clean_state_dict.keys())
    
    if is_baseline_model:
        print("Detected baseline to projection model conversion")
        ...
        for key, value in clean_state_dict.items():
            ...
            # Keep backbone and stem parameters (common to both architectures)
    ...
    if verbose:
        ...
        print(f"Parameters kept for projection model: {len(retained_keys)}")
    ...
    try:
        ...
        print("Successfully converted checkpoint to projection-compatible format")
        print("\nNOTE: You will still see 'missing keys' warnings for the projection layer")
        print("when loading this into the projection model, which is expected behavior.")
        return True
    except Exception as e:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert a baseline model checkpoint to be compatible with projection model")
    parser.add_argument("input", help="Path to input baseline checkpoint file (.pth)")
    parser.add_argument("output", help="Path to output projection-compatible weights file (.pth.tar)")
    parser.add_argument("--quiet", action="store_true", help="Suppress detailed output")
    
    args = parser.parse_args()
    
    success = convert_baseline_to_proj(args.input, args.output, verbose=not args.quiet)
